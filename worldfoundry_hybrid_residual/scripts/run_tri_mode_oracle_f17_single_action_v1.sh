...
ACTION=${1:?usage: run_tri_mode_oracle_f17_single_action_v1.sh Q|C GPU_INDEX}
GPU_INDEX=${2:?usage: run_tri_mode_oracle_f17_single_action_v1.sh Q|C GPU_INDEX}
ACTION=$(printf '%s' "$ACTION" | tr '[:lower:]' '[:upper:]')
if [[ "$ACTION" != Q && "$ACTION" != C ]]; then
    printf 'ACTION must be Q or C\n' >&2
    exit 2
fi
...
OUT="$PROBE_ROOT/results/tri_mode_oracle_f17_${ACTION,,}_single_cells_v1"
mkdir -p "$OUT"
...
"$BASE_ROOT/.venv/bin/python" "$PROBE_ROOT/scripts/build_tri_mode_oracle_schedules.py" \
  --output "$OUT/schedules.json" --actions "$ACTION"
...
"$BASE_ROOT/.venv/bin/python" "$PROBE_ROOT/scripts/generate_wan_tri_mode_oracle.py" \
  ...
  --seeds 20260723 --repeats 2 --frame-num 17 --sampling-steps 20 \
