...
BRANCH=${1:?usage: run_tri_mode_oracle_f17_forecast_probe_v1.sh BRANCH GPU}
GPU_INDEX=${2:?usage: run_tri_mode_oracle_f17_forecast_probe_v1.sh BRANCH GPU}
...
OUT="$PROBE_ROOT/results/tri_mode_oracle_f17_forecast_branch${BRANCH}_probe_v1"
mkdir -p "$OUT"
...
"$BASE_ROOT/.venv/bin/python" "$PROBE_ROOT/scripts/build_tri_mode_oracle_schedules.py" \
  ...
  --probe-steps 14,16,19 --probe-blocks 6,12,24 --branches "$BRANCH" \
  --forecast-scales 0.25,0.5,0.75,1.0
