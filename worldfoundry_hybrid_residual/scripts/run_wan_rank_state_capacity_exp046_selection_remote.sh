...
OUT="$ROOT/results/rank_state_capacity_exp046_gate_v1"
ANALYSIS="$ROOT/results/rank_state_capacity_exp046_gate_v1_analysis"
LOG="$ROOT/logs/exp046_rank_state_capacity_gate_v1.log"
EXIT_FILE="$ROOT/logs/exp046_rank_state_capacity_gate_v1.exit"

if [ ! -f "$ROOT/logs/exp046_rank_state_capacity_smoke_v3.exit" ] || \
   [ "$(cat "$ROOT/logs/exp046_rank_state_capacity_smoke_v3.exit")" != "0" ]; then
  printf 'EXP-046 smoke v3 has not completed successfully\n' >&2
  exit 2
fi
...
if [ -e "$OUT" ] || [ -e "$ANALYSIS" ]; then
  printf 'refusing to overwrite an existing EXP-046 selection artifact\n' >>"$LOG"
  printf '2\n' >"$EXIT_FILE"
  exit 2
fi
...
CUDA_VISIBLE_DEVICES="$GPU" "$PY" \
  ...
  --config "$ROOT/worldfoundry_hybrid_residual/configs/wan_rank_state_capacity_exp046_v1.json" \
  --sample-indices 4,5,6,7 \
  --out-dir "$OUT" \
