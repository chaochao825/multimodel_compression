...
OUT="$PROBE_ROOT/results/worldfoundry_hybrid_f81_confirmation_v1"
...
"$BASE_ROOT/.venv/bin/python" "$PROBE_ROOT/scripts/generate_wan_hybrid_residual.py" \
  ...
  --methods dense,dense_cache008,hybrid,hybrid_cache008 \
  --frame-num 81 --height 480 --width 832 --sampling-steps 20 \
  --residual-targets q,o --residual-blocks all \
  --residual-rank 8 --budget-rank 16 --row-block-size 8 \
  --static-scale-margin 1.05 --teacache-retention-calls 24 \
  --warmup-steps 20 --alternate-method-order \
  >"$OUT/run.log" 2>&1
