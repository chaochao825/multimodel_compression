...
OUT="$PROBE_ROOT/results/worldfoundry_hybrid_smoke_v1"
...
"$BASE_ROOT/.venv/bin/python" "$PROBE_ROOT/scripts/generate_wan_hybrid_residual.py" \
  ...
  --methods dense,fp8,hybrid \
  --frame-num 17 --height 480 --width 832 --sampling-steps 1 \
  --residual-targets q,o --residual-blocks all \
  --residual-rank 8 --budget-rank 16 --row-block-size 8 \
  --static-scale-margin 1.05 --warmup-steps 1 \
  >"$OUT/run.log" 2>&1
