...
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT/results/restricted_rotation_oracle_f81_registered_v1}"
ANALYSIS_DIR="${ANALYSIS_DIR:-$ROOT/results/restricted_rotation_oracle_f81_registered_analysis_v2}"
LOG_DIR="${LOG_DIR:-$ROOT/logs}"
CAPTURE_INDEX="${CAPTURE_INDEX:-$ROOT/results/attention_head_factorial_f81_v1/qkv_replays/capture_index.csv}"
PROTOCOL="${PROTOCOL:-$ROOT/configs/restricted_rotation_oracle_f81_v1.json}"
...
"$PYTHON" scripts/probe_restricted_rotation_oracle.py \
  ...
  2>&1 | tee "$LOG_DIR/restricted_rotation_oracle_f81_registered_v1.log"

"$PYTHON" scripts/analyze_restricted_rotation_oracle.py \
  --input-dir "$OUTPUT_DIR" \
  --output-dir "$ANALYSIS_DIR" \
  2>&1 | tee "$LOG_DIR/restricted_rotation_oracle_f81_registered_analysis_v2.log"
