...
在 H200 被既有任务占用期间，先完成了不计入科学 Gate 的 4090 工程检查：

- rCM 与 native teacher 权重均在加载前匹配冻结身份；
- 官方源码和全部 Python 依赖可以导入；
- 首次检查发现 236 缺少 `google/umt5-xxl` tokenizer 缓存，已作为第 1 次
  有界集成修补，通过 210 镜像建立项目私有离线缓存，未修改共享环境；
- 官方 rCM CLI 使用 CPU UMT5 编码后完成四步 F17 生成，输出为 17 帧、
  `832x480`、16 FPS；首帧、中帧和末帧视觉检查无黑帧或解码异常。

4090 无法同时容纳 GPU UMT5、Wan 和 VAE，因此冻结 runner 的 GPU-text
路径仍必须在 H200 上验证。上述检查只证明 checkpoint、采样方程和解码链路
可执行，不提供 H200 时延、F81 质量或 G-026 通过证据。
