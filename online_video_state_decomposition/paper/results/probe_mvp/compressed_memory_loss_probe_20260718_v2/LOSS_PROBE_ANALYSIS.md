...
- `exact_recent` with `full`: 95/200 (47.5%).
- `learned_recent_query_topk` with `full`: 102/200 (51.0%), net +7 samples.
- `learned_recent_query_topk` with `pca_r256_s4`: 101/200 (50.5%), net -1 sample versus `full`.

The candidate selector changes net correctness by +7 samples at the reference variant. Changing memory from `full` to `pca_r256_s4` changes it by -1 samples. These are descriptive stage deltas, not causal attribution.
