...
RESULT_ROOT="$PROJECT/results/action_dit_transport_cache_20260826"
...
for train_id in 0 1 2; do
  for offset in 8 1; do
    if (( job_index % 2 == SHARD )); then
      run_name="train${train_id}_m${offset}_v1"
      output_dir="$RESULT_ROOT/$run_name"
      if [[ -e "$output_dir" ]]; then
        echo "refusing to overwrite $output_dir" >&2
        exit 1
      fi
      python scripts/probe_action_dit_transport_cache.py \
