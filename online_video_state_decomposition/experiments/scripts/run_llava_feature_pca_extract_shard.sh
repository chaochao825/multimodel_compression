...
if [ "$#" -ne 4 ]; then
  printf 'usage: %s GPU SHARD_INDEX SHARD_COUNT OUT_DIR\n' "$0"
  exit 2
fi
...
split_manifest="${project_root}/configs/mvbench/query_memory_split_20260717.json"
log_dir="${out_dir}/logs"
log_path="${log_dir}/extract_shard_${shard_index}.log"
exit_path="${log_dir}/extract_shard_${shard_index}.exit"
...
flock -n "${lock_path}" python -u \
  "${project_root}/experiments/probes/extract_llava_feature_pca_tokens.py" \
