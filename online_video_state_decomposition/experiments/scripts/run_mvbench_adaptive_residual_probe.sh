...
if [ "$#" -ne 7 ]; then
  printf 'usage: %s GPU SHARD_INDEX SHARD_COUNT OUT_DIR SELECTION_MANIFEST CODEC_PATH SAMPLES_PER_TASK\n' "$0"
  exit 2
fi
...
adaptive_budgets="${ADAPTIVE_RESIDUAL_BUDGETS:-}"
adaptive_per_frame="${ADAPTIVE_RESIDUALS_PER_FRAME_EQUIVALENT:-4}"
adaptive_modes="${ADAPTIVE_RESIDUAL_MODES:-global_energy,temporal_novelty}"
minimum_per_frame="${ADAPTIVE_MINIMUM_PER_FRAME:-0}"
temporal_weight="${TEMPORAL_NOVELTY_WEIGHT:-1.0}"
policies="${POLICIES:-exact_recent,learned_recent_query_topk}"
residual_tokens="${RESIDUAL_TOKENS:-4}"
...
mkdir -p "${log_dir}"
# A preflight failure must never leave a stale success marker from an older run.
printf '125\n' >"${exit_path}"
if [[ ! "${gpu}" =~ ^[0-9]+$ ]]; then
...
command=(
  ...
  --adaptive-residual-budgets "${adaptive_budgets}"
  --adaptive-residuals-per-frame-equivalent "${adaptive_per_frame}"
  --adaptive-residual-modes "${adaptive_modes}"
  --adaptive-minimum-per-frame "${minimum_per_frame}"
  --temporal-novelty-weight "${temporal_weight}"
  --shard-index "${shard_index}"
  --shard-count "${shard_count}"
)
if [ -n "${sample_ids}" ]; then
  command+=(--sample-ids "${sample_ids}")
fi
