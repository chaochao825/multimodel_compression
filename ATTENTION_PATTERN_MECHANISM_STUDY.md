...
- concrete intervention experiments.

It still does not prove the final mechanistic distinction between functional
inductive bias and architectural workaround. That requires the next causal
intervention round: sink masking, register/gate/clipped-softmax variants,
RoPE-axis perturbation for Wan, and `V` subspace stress tests beyond random `V`.
